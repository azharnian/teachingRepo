...
def home():
	...
	tweets = Tweet.query.order_by(Tweet.date_posted.desc()).paginate(per_page=2)
...
def profile():
	...
	tweets = Tweet.query.filter_by(author=current_user).order_by(Tweet.date_posted.desc()).paginate(per_page=5)
	last_page = list(tweets.iter_pages())[-1]
	image_file = url_for('static', filename='profile_pictures/'+current_user.image_file)
	return render_template('profile.html', title="Profile", image_file=image_file, form=form, tweets=tweets, last_page=last_page)
