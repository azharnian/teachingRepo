...
class ContentPage(Frame):
	...
	def __init__(self, parent, Window, *args, **kwargs):
		...
		self.app = Window.app
		self.current_pokemon = self.app.data["Charmender"]
	...
	def header_frame(self):
		...
		self.title_label = Label(self.title_frame, text=self.current_pokemon["fullname"], font=("Arial", 28, "bold"), bg="grey", fg="white")
		...
		char_image = Image.open(self.current_pokemon["image"])
		...
		self.description_label = Label(self.info_frame, text=self.current_pokemon["description"], wraplength=150, justify="left", font=("Arial"), bg="grey", fg="white")
	...
	def footer_frame(self):
		...
		self.types = self.current_pokemon["types"]
		self.type_tabs = []

		self.weaknesses = self.current_pokemon["weaknesses"]
		self.weakness_tabs = []
		...
		self.combobox["values"] = list(self.app.data.keys())
	...
	def selected_combobox(self, event):
		...
		self.current_pokemon = self.app.data[self.combobox["values"][index]]
		# print(self.current_pokemon)
		self.title_label.config(text=self.current_pokemon["fullname"])
		self.description_label.config(text=self.current_pokemon["description"])
		...
		self.char_image = ImageTk.PhotoImage(char_image)
		self.char_image_label.config(image=self.char_image)

		self.types = self.current_pokemon["types"]
		self.type_tabs = []

		for label in self.type_tab.grid_slaves():
			label.grid_forget()

		for label in self.weakness_tab.grid_slaves():
			label.grid_forget()
