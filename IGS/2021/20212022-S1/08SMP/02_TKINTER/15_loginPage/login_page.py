...
class LoginPage(Frame):
	...
	def create_login_frame(self):
		...
		self.label_username = Label(self.login_frame, text="Nama Pengguna", font=("Arial", 16), bg="white")
		...
		self.entry_username = Entry(self.entry_username_frame, font=("Arial", 16), textvariable=self.entry_username_var)
		self.entry_username.pack(fill="both", expand=True)

		self.label_password = Label(self.login_frame, text="Kata Sandi", font=("Arial", 16), bg="white")
		...
		self.entry_password = Entry(self.entry_password_frame, font=("Arial", 16), show="*", textvariable=self.entry_password_var)
		self.entry_password.pack(fill="both", expand=True)

		self.login_button = Button(self.login_frame, text="Masuk", font=("Arial", 16), command=self.click_login_button)
		self.login_button.grid(row=2, column=0, columnspan=2, sticky="nsew", padx=100, pady=10)


	def click_login_button(self):
		username = self.entry_username_var.get() # translate ui tkinter -> original string
